...
colours. The font only uses colour 15 (white), so PAL3's colours 1-14 carry the hero portrait's
palette (bust on the creation screen, avatar in the panel); every portrait palette keeps 15 white.
Palettes: PAL0 view, PAL1 companion avatars, PAL2 figures (one kind at a time), PAL3 text + hero. In-game text is German: `res/gfx/font_de.png` (generated by `tools/make_font.py` from
...
src/
  ...
  portraits.c/.h         The hero portraits per class (creation screen, panel avatar)
...
res/
  ...
  gfx/portrait_*.png       Hero portraits, three per class: 80x96 busts for the creation screen
  gfx/hero_*.png           ...and the matching 24x24 panel avatars (one palette per portrait, PAL3)
  gfx/avatar_wir.png       "Wir" avatar (brain on four legs), companion avatars share its palette
  gfx/avatar_laezel.png    Lae'zel avatar, same palette (also avatar_shadowheart.png)
  gfx/fig_*.png            Figures: imp (3 frames), Lae'zel and Schattenherz (full figure and an
                           80x96 close-up bust shown while they speak), target marker
...
tools/
  ...
  make_avatar.py           Regenerates the avatar sprites (24x24, shared palette; the companions'
...
- **In the fight**, the enemies stand in front of the party as figures: hardware sprites on PAL2
  (`src/figures.c`, `tools/make_figures.py`), animated (the imps' wing beat), blinking when hit;
  the view shakes when the party is hit. Every figure brings its own palette, loaded into PAL2
...
XGM2 driver (`src/sfx.c`, pattern from the Wanderburg project): PSG music generated by
`tools/generate_music.py` (VGM files, `XGM2 music_x "music/x.vgm"`), 8-bit PCM effects
synthesised by `tools/generate_sfx.py` (`WAV sfx_x "sfx/x.wav" XGM2`).

- Music: title (through the creation screen), dungeon (rooms 1-5), combat, bridge (Room 6),
  ending. `music_playRoom` picks the room's song on arrival, `encounter_fight` switches to the
...
See [TODO.md](TODO.md) for what's next (more spells and enemies, real art).
