...
- `tools/emutest.py` — drives BlastEm's debugger (`-d`) to play the ROM headlessly and save
  ...
  fight -- dice decide, so check the screenshots).
...
res/
  ...
  gfx/avatar_laezel.png    Lae'zel avatar, same palette
  gfx/fig_*.png            Figures: imp (3 frames), Lae'zel, target marker (shared palette)
...
(which stays shut until Room 4 exists).
...
- Everyone acts in initiative order (d20 + DEX, rolled once per fight). The player picks every
  ...
  die twice), Geschoss (Magier, 2 ZP, never misses), Heiltrank (any member, revives the fallen),
...
See [TODO.md](TODO.md) for what's next (Rooms 4-6, more spells and enemies, real art).
